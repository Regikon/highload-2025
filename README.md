...
| Технология | Область применения  | Мотивация                                                                                                                                           |
|------------|---------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------|
| Prometheus | БД метрик           | Специализированная база данных, позволяющая хранить метрики приложений. Клиентский протокол на базе HTTP, что делает простым внедрение в приложение |
| Grafana    | Дашборды для метрик | Предоставляет удобный веб-интерфейс с графиками метрик, способен собирать метрики из различных БД.                                                  |
...
| Rust       | Язык для медиасервера      | Потокобезопасный, а также безопасный с точки зрения доступа к памяти язык из коробки, есть асинхронный рантайм, высокая производительность за счет отсутствия накладных расходов на сборку мусора и иных прелестей рантайма Golang |
